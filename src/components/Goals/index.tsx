...
const Goals = () => {
    const entries = [
        { icon: faCrosshairs, label: "Goals", color: "green" },
        { icon: faBowlFood, label: "Popular Dishes", color: "purple" },
        { icon: faPlateWheat, label: "Menus", color: "red" }
    ]
        ...
        {entries.map(entry =>
            <div className="flex items-center p-4 gap-4 justify-between">
                <div className="flex items-center">
                    <FontAwesomeIcon className={`mr-4 rounded-full size-7 p-3 bg-${entry.color}-300/90`} icon={entry.icon} color={entry.color} />
                    <b>{entry.label}</b>
                </div>
